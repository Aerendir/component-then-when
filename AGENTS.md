...
When using Flex, to the `cpomser.json` file is added the key `extra.symfony.require` with the version of Symfony to install, e.g. `"^7.0"`. This is used by Flex to determine which version of Symfony to install and to update the `symfony/*` packages accordingly.
...
- `rector.php`: update the `SymfonySetList` sets to the lowest supported version, e.g. `SymfonySetList::SYMFONY_54`. Read the class `SymfonySetList` and use all existing sets for the supported lowest version. If some sets are deprecated, ignore the deprecation and use the set anyway.
